...
### Test-isolation debt (M0 findings, 2026-06-24)

M0 stabilization took the suite from **19 → 3 failures** (486 passed). The 3 residuals are
**pre-existing test-infrastructure debt**, not app breakage (the MVP loop E2E passes). Fixes applied:
`.env`/`.env.example` restored vault sandbox protection (`THREAD_ALLOW_TEST_PROMOTE=false`);
